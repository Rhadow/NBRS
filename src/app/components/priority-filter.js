...
PriorityFilter = React.createClass({
	...
	componentWillUpdate: function(nextProps, nextState) {
        if(nextProps.selectedProjectName !== this.props.selectedProjectName){
            this._selectedPriority = constants.CH_LEXICON.PRIORITY_ALL;            
		    this.props.onSelectHandler(this._selectedPriority);
        }
    },
	...
	render: function() {
		return (
			...
			<div className="priority-filter">
			    <label>{constants.CH_LEXICON.PRIORITY_TITLE}</label>
	            <select 
	                ref="priorityFilter" 
	                onChange={this._handleSelect} 
	                value={this._selectedPriority}>
	                <option>{constants.CH_LEXICON.PRIORITY_ALL}</option>
	                <option>{constants.CH_LEXICON.PRIORITY_LOW}</option>
	                <option>{constants.CH_LEXICON.PRIORITY_MEDIUM}</option>
	                <option>{constants.CH_LEXICON.PRIORITY_HIGH}</option>
	                <option>{constants.CH_LEXICON.PRIORITY_SOLVED}</option>
	            </select>
			</div>			
            /* jshint ignore:end */
		);
